...
fn networkNode(p: vec2f) -> f32 {
  let q = rotate(p, -0.22);
  let outer = sdRoundedBox(q, vec2f(0.20, 0.16), 0.028);
  let inner = sdRoundedBox(q, vec2f(0.15, 0.11), 0.018);
  let cellA = sdRoundedBox(q - vec2f(-0.095, -0.055), vec2f(0.035, 0.025), 0.007);
  let cellB = sdRoundedBox(q - vec2f(0.095, 0.055), vec2f(0.035, 0.025), 0.007);
  var node = opSmoothSub(outer, inner, 0.008);
  node = opSmoothUnion(node, cellA, 0.008);
  node = opSmoothUnion(node, cellB, 0.008);
  return node;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let nodeP = (p - vec2f(0.46, -0.12)) / breathe;
  let dNode = networkNode(nodeP);
  let fill = 1.0 - smoothstep(-0.0015, 0.004, dNode);
  let line = 1.0 - smoothstep(0.0, 0.0055, abs(dNode));
  let glow = exp(-max(dNode, 0.0) * 16.0);
  ...
  let blue = vec3f(0.0, 0.294118, 0.431373);
  let teal = vec3f(0.172549, 0.611765, 0.572549);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(blue, mix(teal, paper, 0.56), spark * 0.45 + line * 0.05);
